...
var gFormat = createFormat({prefix:"G", width:2, zeropad:true, decimals:1}); // temp until fanuc probing is refactored
...
var mClampModal = createModalGroup(
  {strict:false},
  [[11, 10], // 4th axis LOCK / UNLOCK
    [51, 50]], // 5th axis LOCK / UNLOCK
  mFormat
);
...
function onCycleEnd() {
  if (isProbeOperation()) {
    gMotionModal.reset();
    zOutput.reset();
    // TODO: add protected retract? see fanuc
  }
    if (!cycleExpanded) {
...
function onCommand(command) {
  ...
  case COMMAND_LOCK_MULTI_AXIS:
    ...
    writeBlock(mClampModal.format(51));
    return;
  case COMMAND_UNLOCK_MULTI_AXIS:
    writeBlock(mClampModal.format(10));
    writeBlock(mClampModal.format(50));
    return;
...
function writeRetract() {
  ...
  if (retract && retract.words.length > 0) {
    ...
    for (var i in retract.words) {
      ...
      case "M26":
        if (retract.retractAxes[2]) { // Z retract
